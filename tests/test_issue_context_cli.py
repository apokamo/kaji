...
class TestContextCLISmall:
    ...
    def test_gitlab_provider_accepts_context_via_provider_method(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """Red→Green 再現: ``provider.type='gitlab'`` 配下で ``kaji issue context``
        が受理され、``GitLabProvider.resolve_issue_context()`` 経由で IssueContext
        を返すこと。

        gl:7 で dispatcher gate 撤去前は ``EXIT_INVALID_INPUT`` (=2) を返していた。
        """
        repo = tmp_path / "repo"
        (repo / ".kaji").mkdir(parents=True)
        (repo / ".kaji" / "config.toml").write_text(
        ...
        stub = IssueContext(
            ...
            branch_prefix="fix",
            branch_name="fix/6",
            worktree_dir="/tmp/kaji-fix-6",
            design_path="draft/design/issue-6-some-slug.md",
            provider_type="gitlab",
            branch_prefix_fallback=False,
            default_branch="main",
        )
        with patch(
            "kaji_harness.providers.GitLabProvider.resolve_issue_context",
            return_value=stub,
        ) as mock_resolve:
            rc = _handle_issue(["context", "6"])
        assert rc == 0
        mock_resolve.assert_called_once_with("6")
        out = json.loads(capsys.readouterr().out)
        assert out["provider_type"] == "gitlab"
        assert out["issue_ref"] == "gl:6"
        assert out["branch_name"] == "fix/6"

    def test_gitlab_provider_accepts_gl_prefix(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """``gl:N`` 形式入力も bare ``N`` と同じく受理され、剥がして渡される。"""
        repo = tmp_path / "repo"
        (repo / ".kaji").mkdir(parents=True)
        (repo / ".kaji" / "config.toml").write_text(
            '[paths]\nartifacts_dir = ".kaji-artifacts"\nskill_dir = ".claude/skills"\n\n'
        ...
        with patch(
            ...
            rc = _handle_issue(["context", "gl:6"])
        assert rc == 0
        # gl: prefix は剥がして渡る
        mock_resolve.assert_called_once_with("6")

    def test_gitlab_provider_rejects_cross_provider_id(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """cross-provider 参照 (``gh:N``) は ``EXIT_INVALID_INPUT`` で reject。"""
        repo = tmp_path / "repo"
        (repo / ".kaji").mkdir(parents=True)
        (repo / ".kaji" / "config.toml").write_text(
            '[paths]\nartifacts_dir = ".kaji-artifacts"\nskill_dir = ".claude/skills"\n\n'
        ...
        monkeypatch.chdir(repo)
        rc = _handle_issue(["context", "gh:6"])
        assert rc == 2
        captured = capsys.readouterr()
        assert "gh:" in captured.err

    def test_gitlab_provider_error_is_normalized_to_runtime_error(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """GitLab 経路で ``GitLabProviderError`` が漏れず ``EXIT_RUNTIME_ERROR`` に正規化される。"""
        ...
        with patch(
            "kaji_harness.providers.GitLabProvider.resolve_issue_context",
            side_effect=GitLabProviderError("glab boom"),
        ):
            rc = _handle_issue(["context", "6"])
        assert rc == 3  # EXIT_RUNTIME_ERROR
        captured = capsys.readouterr()
        assert "glab boom" in captured.err
