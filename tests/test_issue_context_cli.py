...
class TestContextCLISmall:
    ...
    def test_gitlab_provider_rejected(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """設計契約: ``provider.type='gitlab'`` 配下では ``kaji issue context`` を未対応として拒否。"""
        repo = tmp_path / "repo"
        (repo / ".kaji").mkdir(parents=True)
        (repo / ".kaji" / "config.toml").write_text(
            '[paths]\nartifacts_dir = ".kaji-artifacts"\nskill_dir = ".claude/skills"\n\n'
        ...
        monkeypatch.chdir(repo)
        rc = _handle_issue(["context", "1"])
        assert rc == 2
        captured = capsys.readouterr()
        assert "gitlab" in captured.err.lower()
