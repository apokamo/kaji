...
- **検証期間の終了基準**: forge 採用先（gitlab 本格採用 / github 復帰 / 他）が
...
**現状**: GitLab に push（バックアップ兼ねる、急ぎ採用）。本格採用判断は別途。
本章は GitLab を「現状の選択」として記述するに留め、推奨明示はしない。
...
GitLab Cloud に push するための一般的な手順を以下に示す。詳細な GitLab 操作
は GitLab 公式ドキュメントを参照すること。

1. GitLab に private project を作成（例: `apokamo/kaji`）
2. SSH 鍵を GitLab アカウントに登録（`ssh-keygen` → `cat ~/.ssh/id_ed25519.pub`
   を GitLab Settings > SSH Keys に追加）
3. 既存 kaji repo に GitLab remote を追加:
   ```bash
...
4. 初回 push:
   ...
   ```
5. 以降は通常運用で `git push gitlab main`（または default upstream を gitlab に
   切り替え）

> **注**: 本手順は一般的な GitLab セットアップを記述したもの。kaji repo の
> GitLab ミラー設定の作業記録は `draft/lab/gitlab/setup-log.md` にあるが、
> 本 runbook は当該作業記録に依存せず独立した参照として執筆している。

### 4.3 採用判断の基準

検証期間中は GitLab 継続で十分。本格採用 / 切替の判断材料：
...
> GitLab を primary forge として採用する場合のセットアップ / 必須前提
> （Merge method / Squash 設定 / `glab` 認証）/ `provider.type='gitlab'` 起動
> 手順は [GitLab Mode CLI Guide](../cli-guides/gitlab-mode.md) を参照。本節は
> 移行判断材料に focus する。
...
- 移行時に必要な kaji 実装は `draft/design/local-mode/design.md` § 残課題 を
  参照。なお、`kaji sync from-gitlab`（GitLab Issue cache populate）と
  `kaji sync status` は issue `local-pc5090-8` で実装済み。`kaji sync from-github`
  / `kaji sync local-to-github-plan` / PR context 注入 等は引き続き残課題
