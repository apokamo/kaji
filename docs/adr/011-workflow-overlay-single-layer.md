...
- **保存タイミングは run 開始時**（`WorkflowRunner.run()` 内、run_dir を採番する
  ...
  artifact が欠落するためである。
- したがって差し込み点は `runner.py:938-939` 近傍の 1 箇所であり、`commands/run.py:159-181`
  （`load_workflow` → provider-match 検証 → `WorkflowRunner` 生成）は overlay 対応のための
  変更を要しない。
...
| run artifact 保存 | `kaji_harness/runner.py:938-939` 近傍への追加 | 必須 |
...
無変更で済むのは `kaji_harness/models.py` の `Workflow` / `Step` dataclass と、
`commands/run.py` の workflow ロード〜`WorkflowRunner` 生成経路、および既存 33 本の YAML である。
`runner.py` は overlay の解決には関与しないが、provenance artifact のために 1 箇所変更する。
