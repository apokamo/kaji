...
flowchart TB
    ...
    E["/issue-sync-docs"] --> F["/issue-pr"] --> G["/issue-close"]
```
...
┌─────────────────────────────────────────────────────────────────────┐
│ Phase 5: ドキュメント同期                                           │
│ /issue-sync-docs <issue-number>                                     │
├─────────────────────────────────────────────────────────────────────┤
│ • Issue本文からWorktree情報を取得 → 移動                            │
│ • draft/design/ の内容を確認                                        │
│ • チェックリストに基づき関連ドキュメントの更新要否を判断            │
│   - ADR: 技術選定・アーキテクチャ変更の決定                         │
│   - architecture.md: システム構成・コンポーネント変更               │
│   - guides/: 開発者向け手順・ワークフローの追加                     │
│ • 必要に応じてドキュメント更新 + コミット                           │
│ • Issueにドキュメント同期完了コメント                               │
└─────────────────────────────────────────────────────────────────────┘
                                  ↓
┌─────────────────────────────────────────────────────────────────────┐
│ Phase 6: PR作成                                                     │
...
| `/issue-sync-docs` | 設計書と関連ドキュメントの整合性確認・更新 |
