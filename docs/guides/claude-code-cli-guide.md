...
| `--permission-mode delegate` | 権限委譲モード |
...
| オプション | 説明 | 例 |
|-----------|------|-----|
| `--max-budget-usd` | API費用の上限（USD） | `--max-budget-usd 1.0` |
| `--fallback-model` | 過負荷時のフォールバックモデル | `--fallback-model haiku` |
| `--json-schema` | 構造化出力のJSONスキーマ | `--json-schema '{"type":"object"}'` |
| `--input-format` | 入力形式（text/stream-json） | `--input-format stream-json` |
| `--no-session-persistence` | セッション永続化を無効化 | `--no-session-persistence` |
...
| 日付 | 内容 |
|------|------|
| 2026-01-13 | v2.1.6対応: delegate モード追加、新オプション追加 |
| 2025-11-27 | 初版作成 |
