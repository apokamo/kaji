...
Conventional Commits 準拠の `type:*` 11 ラベル + 直交 meta 8 ラベル（合計 **19 ラベル**）を `.github/labels.yml` で宣言的に管理し、GitHub Actions（`labels-sync.yml`）で repo に同期する。配色は Catppuccin Mocha パレット。旧ラベル（`enhancement`, `bug`, `documentation`, `refactoring`, `e2e-test`）は削除し、OPEN Issue は新ラベルへ一括再マッピング済み。
...
OPEN Issue（実施時 9 件）は手動で再マッピング。CLOSED Issue は scope 外（コスト > 効果）。
...
- **複数 `type:*` 付与**: 許可するが、主たる 1 つを推奨（`docs+test` 等の混在対応）
...
本 RFC を実装した #154 自体は、`type:*` 体系を新設する Issue であるため、`type:*` ラベル体系成立前は付与不能だった。実装途中で `type:chore` を bootstrap 用途として先行作成し、Issue に付与することで `issue-review-design` の cardinality チェックを通した。完了時の self-validation で `enhancement` を剥がして `type:chore` のみが残ることを確認している。
