...
- `kaji pr merge` は `--squash` / `--rebase` flag を **kaji 側で silent に除去**し、常に `gh pr merge --merge` 固定で叩く（`--no-ff` only の merge 規約）。`--admin`（管理者 bypass）と `--match-head-commit <SHA>`（HEAD 拘束）は method flag ではないため除去されず、強制付与される `--merge` とともに `gh pr merge` へ転送される
- `kaji pr admin-merge-check <branch>` は **read-only** の self-PR admin merge 適格性判定で、`/issue-close` が通常 merge を base branch policy（`required_review_thread_resolution=true` 下の未解決 review thread 等）で block された場合に使う。GitHub への write は行わない。次の 4 条件を **すべて** 満たす場合だけ ALLOW（exit 0、解決した 40 桁 hex の HEAD SHA を stdout に 1 行出力）とし、それ以外はすべて DENY（exit 非 0、stdout 空、stderr に `DENY: <理由>`）する:
  1. **self-PR** — PR author == authenticated user
  2. **fresh APPROVED marker** — 全 issue comment（`gh api --paginate --slurp` で取得）から抽出した最新の `kaji-review` decision marker（`<!-- kaji-review: state=APPROVED sha=<40-hex> -->`）が、**authenticated user 自身が投稿**したものであり、かつ marker の `sha` が現在の `headRefOid` と一致する。他者が投稿した marker、`sha` が現在 HEAD と不一致な marker（force-push 後に残った stale approval。backdated commit への force-push を含む）、現在 HEAD に対する新しい `CHANGES_REQUESTED`、marker 不在はすべて DENY。改竄可能な `committedDate` ではなく HEAD SHA と投稿者に束縛することで、marker 詐称と stale approval bypass の両経路を塞ぐ
  3. **policy-block 適格** — `mergeStateStatus == BLOCKED` かつ `mergeable == MERGEABLE` かつ `statusCheckRollup` に失敗 / 保留 check を含まない。transient / auth / conflict / 非 policy 失敗を elevated merge から除外する
  4. **admin bypass** — authenticated user の `permissions.admin == true`

  途中の `gh` エラー / JSON parse 失敗 / PR の曖昧解決（head に対する open PR が 0 件 / 複数件）/ 不正な `headRefOid` はすべて fail-closed で DENY。`/issue-close` は exit code で分岐し、出力された SHA を `kaji pr merge <branch> --admin --match-head-commit "$HEAD_SHA"` に渡す（check 後に HEAD が動いていれば GitHub 側が merge を拒否する）。`--auto` へ fallback せず、review thread の Resolve も行わない
- `kaji pr review <pr> --approve` / `kaji pr review <pr> --request-changes` は self-PR (PR author == authenticated user) を検知すると `<!-- kaji-review: state=APPROVED sha=<head> -->` / `<!-- kaji-review: state=CHANGES_REQUESTED sha=<head> -->`（`<head>` は現在の `headRefOid`）marker 付き comment を Issue comments API に投稿することで review シグナルを表現し rc=0 を返す。`gh pr review --approve` / `--request-changes` は GitHub API が author の APPROVE / REQUEST_CHANGES event を `Can not approve your own pull request` / `Can not request changes on your own pull request` で 422 拒否するため、self-PR では skip される。非 self-PR では従来通り `gh pr review --approve` / `--request-changes` を委譲する。`--comment` / flag 無しは routing 段で `_github_pr_review` に分岐せず従来通り `gh pr review` へ passthrough
