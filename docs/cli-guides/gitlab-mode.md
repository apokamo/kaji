...
GitLab provider の実通信 E2E（子 Issue `local-pc5090-10` 範囲）を回す場合の
前提:
...
- 必要に応じて `KAJI_TEST_GITLAB_REPO=<group>/<project>` 等の env を設定
  （実装は子 Issue `local-pc5090-10` で確定。本ガイドは前提条件側のみ記述）

> 注: `make test-large-gitlab` ターゲット自体は子 Issue `local-pc5090-10` で
