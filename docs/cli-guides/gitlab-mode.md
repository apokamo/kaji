...
### 5.7 commit body の `Fix #N` / `Fix [N]` 等が無関係 GitLab issue を auto-close する

GitLab は commit message / MR description / Issue note 内の `Closes #N` /
`Fix(es) #N` / `Resolves #N` / `Implements #N` 系（大小区別なし、長い文字列内の
部分一致でも有効）を **auto-close keyword** として解釈し、push 時 / MR merge
時に当該 project 上の issue #N を自動 close する（公式仕様:
[GitLab Default closing pattern](https://docs.gitlab.com/ee/user/project/issues/managing_issues.html#default-closing-pattern)）。

kaji の review workflow 慣用の `Must Fix [N]`（review item index、N=1,2,3,...）も
**`Fix [N]` 部分が `Fix #N` 等価として match** し、push した瞬間に当該 GitLab
project の issue #N が close される。実発生例: smoke test (gl:1) で 171 commits
を push した際、commit body 末尾の `Must Fix [N]` 表記により tracking issue
gl:1 が意図せず close 状態になった（commit `3760ae2`）。

**回避ルール**（`provider.type='gitlab'` 配下で必須）:

- commit body / MR description / `kaji issue comment` 本文には `#N` の生表記を
  書かない。review item の参照は `Must Fix item N` / `Must Fix 指摘 N` /
  `point N` 等で代替する
- close keyword + 数字の組合せが必要なときは数字を `<N>` placeholder にする
...
skill / workflow 側の規約は [docs/dev/shared_skill_rules.md § GitLab
auto-close keyword 回避](../dev/shared_skill_rules.md#gitlab-autoclose-keyword-回避)
に集約する。
