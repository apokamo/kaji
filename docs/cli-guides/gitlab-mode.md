...
> ⚠️ **destructive default behavior 注意**: GitLab は commit message および
> MR description 内の close keyword（`Closes` / `Fix` / `Resolve` / `Implements`
> 等）+ `#<数値>` を auto-close keyword として解釈し、default branch への push /
> MR merge で **無関係な issue を自動 close する**。トラブルシューティングの
> § 5.7 に分類しているが、これは「動かない」系ではなく「気づかず動いて issue が
> 消える」destructive default。skill / workflow から commit / MR description を
> 生成する場合は必ず本ガイド § 5.7 と
> [docs/dev/shared_skill_rules.md § GitLab auto close keyword 回避](../dev/shared_skill_rules.md#gitlab-auto-close-keyword-回避)
> の placeholder / grep 規約に従うこと。
...
（大小区別なし、word boundary で検出。`Affixed` 等の単語内部分一致は対象外だが、
space / 句読点で区切られた独立 token として keyword が現れた場合は文中のどこに
あっても match する）を **auto-close keyword** として解釈し、commit が default
branch に push された時点 / MR が merge された時点で当該 project の issue を
自動 close する（公式仕様:
...
- push 前に以下 grep で hazard pattern を検出する。正規表現は GitLab spec の
  closing pattern（`Close[sd]?` / `Closing` / `Fix(es|ed|ing)?` /
  `Resolve[sd]?` / `Resolving` / `Implement(s|ed|ing)?`）をすべて拾える形にする:
  ```bash
  ...
  > は `e[sd]?` の `e` が必須となるため `Fix #N` / `Closing #N` /
  > `Implementing #N` 等の **本 incident のトリガー pattern を検出できない**。
  > 旧 regex を流用しないこと。
- push 後は意図しない close が発生していないか確認する。具体手順:
  ```bash
  # 1. 現在開いている issue 一覧を確認（push 前との差分で消えた N を特定）
  glab issue list --repo <group>/<project> --state opened

  # 2. 該当 issue の close 経緯を確認（system note に "closed by ..." が残る）
  glab issue view <N> --repo <group>/<project> --comments

  # 3. 原因 commit / MR を特定したうえで reopen
  glab issue reopen <N> --repo <group>/<project>
  ```
  reopen 後、原因 commit を amend するか follow-up commit で hazard 表記を
  placeholder 化し、再発防止を docs / skill 側に反映する
