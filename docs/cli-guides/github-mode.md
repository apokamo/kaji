...
> `Resolves #<N>`, and similar phrases in PR descriptions and commit messages as
> auto-close keywords. **By default** the referenced issue is closed automatically
> when the PR is merged into the default branch (official docs:
...
> (Settings → General → Features → Issues), so the default behavior does not apply
> here. On that premise, `/i-pr` emits exactly one live `Closes <issue_ref>` line in
> the PR description (the only automatable way to link a PR to an issue), while
> closing keywords remain forbidden in commit bodies and elsewhere in the PR
> description. Issues are closed explicitly via `/issue-close`.
>
...
the referenced issue automatically on merge to the default branch **unless** the
repository setting **Auto-close issues with merged linked pull requests** is
disabled
