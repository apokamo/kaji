...
`/issue-implement` は開始時に [implement-quickref.md](./implement-quickref.md) を読み、正本規約を状況依存で部分 Read する。Baseline Check と Pre-Handoff Review の詳細手順、および実装完了報告 template は責務を維持したまま skill 配下へ分離し、それぞれ該当 Step で遅延読込する。
...
two-way door は仮定と検査先を明記して進める。公開 CLI/API、永続化 schema、migration、
運用、非互換変更、ユーザー価値などの one-way door が人間未決なら `ABORT` し、
決めるべき項目を返す。決定済みだが記述や provenance が不足しているだけなら `RETRY`
で補完する。正本は
