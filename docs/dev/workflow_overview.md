...
| dev workflow + マージ後の品質確認まで自動化したい | feature-with-postmerge | dev workflow + `wait-merge` → `verify-main-green` → `post-merge-review` → `post-merge-close` |

> `feature-with-postmerge.yaml` は `feature-development.yaml` と独立したワークフローで、PR マージ後に main ブランチの CI green 確認とコミット範囲レビューを行ってから Issue をクローズする。`feature-development.yaml` はそのまま維持しているため、既存ユーザーには影響しない。
