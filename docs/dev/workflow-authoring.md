...
| `inject_verdict` | bool | — | `true` で前ステップの verdict を注入（`resume` なしでも使用可） |
...
## inject_verdict（verdict 注入）

`resume` なしで前ステップの verdict を受け取りたい場合に使用する。mixed-agent 構成（例: review を codex、fix を claude で実行）で、セッション継続は不要だが verdict は引き継ぎたいケースに対応する。
...
steps:
  ...
  - id: fix-code
    skill: fix-code
    agent: claude
    inject_verdict: true      # review-code の verdict を受け取る（resume 不要）
    on:
      PASS: verify-code
```
