...
- **孤立サロゲートガード**: high のみ / low のみが残ると Python str としては成立するが `encoding="utf-8"` での書き出しで `UnicodeEncodeError` を起こすため、その場合は復号結果を捨てて原文（`\uD83D` 等のリテラル）をそのまま返す。これにより `cli.py:257` の `open(log_dir / "console.log", "a", encoding="utf-8")` パスが破壊されない
...
> **表示経路 2 sink のカバレッジ根拠**: `stream_and_log`（`cli.py:282-288`, `:314`）は `adapter.extract_text(event)` の戻り値を `texts` に集約し、`console.log`（`f_con.write`）と `CLIResult.full_output`（`"\n".join(texts)`）の **両方** に同一文字列を流す。したがって `CodexAdapter.extract_text` の戻り値を assert する上記テストは、Issue 完了条件が要求する「`console.log` / `CLIResult.full_output` の双方に `config/workflow の暗黙` が含まれ、OB 節に示した未デコードの Unicode escape リテラル（バックスラッシュ + `u` + 16 進 4 桁表記）が残らない」ことを共通上流で担保する。両 sink を個別に叩く追加の統合テストは不要。
