...
| `kaji_harness/` 内 `gitlab\|glab\|gl:` 参照（file 数、大小文字無視） | **16 ファイル** | `grep -rln -i "gitlab\|glab\|gl:" kaji_harness/ --include="*.py" \| wc -l` | 1（`kaji_harness/sync.py` のみ。legacy cache migration detector に限定。後述「許容除外規則」参照） |
| 同（総ヒット数） | **451 件** | `grep -rcn -i "gitlab\|glab\|gl:" kaji_harness/ --include="*.py"` 合計 | ≤ 8（`sync.py` 内 migration detector + recovery エラー文に限定） |
...
| `docs/` 内 GitLab 言及 markdown（大小文字無視） | **14 ファイル** | `grep -rln -i "gitlab\|glab" docs/ --include="*.md" \| wc -l` | 1（`docs/adr/` 配下の撤去 ADR 1 本に限定。後述「許容除外規則」参照） |
...
`kaji sync status` 起動時の legacy GitLab cache 検出 fail-fast は、**`kaji_harness/sync.py` 内の単一関数 `_detect_legacy_gitlab_cache(cache_dir)`（実装時に命名確定、機能は § Cache artifact 移行ポリシー 参照）と、そこから raise する `SyncError` メッセージ文字列**に閉じる。それ以外の `kaji_harness/` ファイルでは **0 件**。

許容ヒットの厳密内訳（実装時に diff レベルで担保。**合計 8 hits 以内**）:

| 出現位置 | リテラル | ヒット数 |
|---------|---------|---------|
| `_detect_legacy_gitlab_cache()` 関数定義 + caller | 関数名 `_detect_legacy_gitlab_cache` | 2 hits（def 行 + caller 1 箇所） |
...
grep -rln -i "gitlab|glab|gl:" kaji_harness/ --include="*.py" | grep -v "^kaji_harness/sync.py$" | wc -l   # → 0

# (b) sync.py 内のヒット総数が 8 以内かつ migration detector に閉じること
grep -cn -i "gitlab|glab" kaji_harness/sync.py                              # → ≤ 8
grep -n "_detect_legacy_gitlab_cache" kaji_harness/sync.py | wc -l          # → ≥ 2（定義 + caller）
...
grep -rln -i "gitlab|glab" docs/ --include="*.md" | grep -v "^docs/adr/" | wc -l   # → 0
```
...
これらの扱いを以下方針で定義する。**legacy cache 検出は `_detect_legacy_gitlab_cache(cache_dir)` 単一関数に集約**し、`kaji sync status` / `view_cached_*_issue` / list 統合経路の **すべての entry point** から先頭で呼び出して fail-fast する。検出条件は (a) `.sync-meta.json` の `forge == "gitlab"` または (b) `cache_dir/gl-*.json` の存在 のいずれかで成立する **OR 結合**（MF-2 round 3 反映、meta 不在 + `gl-*.json` 単独残存ケースを明示処理）:
...
| `kaji_harness.providers.local.LocalProvider.view_cached_*_issue()` | 関数冒頭、cache 読込の **前**（LocalProvider 側からは `kaji_harness.sync._detect_legacy_gitlab_cache()` を import して呼ぶ。重複実装は避ける）|
...
3. `kaji_harness/providers/local.py`:
   ...
   - 関連 docstring 言及を削除
   - `view_cached_*_issue()` 系および `list_issues()` の cache 統合経路冒頭に `kaji_harness.sync._detect_legacy_gitlab_cache(self.cache_dir)` を呼ぶ guard を追加（MF-2 round 3、`gl-*.json` 単独残存ケースで silent regression を防ぐ）。**detector は sync.py に閉じ、local.py 側には GitLab リテラルを残さない**（import 文と関数呼び出しのみ）
...
- `grep -rln -i "gitlab\|glab\|gl:" kaji_harness/ --include="*.py" | grep -v "^kaji_harness/sync.py$" | wc -l` → **0**（sync.py を除外して評価。許容除外規則 § 2）
- `grep -cn -i "gitlab\|glab" kaji_harness/sync.py` → **≤ 8**（migration detector + recovery エラー文）
- `grep -n "_detect_legacy_gitlab_cache" kaji_harness/sync.py | wc -l` → **≥ 2**（定義 + caller、detector 関数が実在することの確認）
...
- `grep -rln -i "gitlab\|glab" docs/ --include="*.md" | grep -v "^docs/adr/" | wc -l` → **0**（撤去 ADR を除外して評価。許容除外規則 § 3）
...
`_detect_legacy_gitlab_cache()` の 3 ケース parametrize で legacy cache 検出 fail-fast をカバーする（`tests/test_sync_from_github.py` または新規 `tests/test_legacy_gitlab_cache_detection.py` に追加）。
