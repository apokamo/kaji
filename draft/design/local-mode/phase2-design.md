...
**E2E 動作確認は Large 自動テストではなく手動 smoke として扱う**: 改修者がローカルで `provider=github` の代表経路（`issue-start` / `issue-design`+`issue-implement` / `i-pr` / `pr-fix` または `pr-verify` / `issue-close` の 5 経路）を動かして Skill が壊れていないことを確認する。1 step だけでは 20 Skill の置換リスクをカバーできない。これは受け入れ条件の手動チェック項目とし、CI には載せない（詳細は受け入れ条件「手動 smoke（複数経路）」参照）。
...
- [ ] **手動 smoke（複数経路）**: 改修者がローカルで以下の代表経路を `provider=github` で動かし、Skill 改修によるレグレッションが無いことを確認する。1 step だけでは 20 Skill の置換リスクをカバーできないため複数経路を要求する（CI 自動化はしない、Large 分類のため）
  - `/issue-start` 経由の worktree 作成（`[prefix]/[issue_id]` のリネーム経路）
  - `/issue-design` → `/issue-implement` の 1 サイクル（`[issue_id]` 引数透過の基本動作）
  - `/i-pr` の PR 作成（`gh pr create` → `kaji pr create` 置換、`Closes [issue_ref]` 展開、`CURRENT_BODY=$(...)` ラウンドトリップ）
  - `/pr-fix` または `/pr-verify` で 1 件のレビューコメント取得（`kaji pr review-comments` / `reviews` の新コマンド経路）
  - `/issue-close` の PR merge + worktree 削除（`kaji pr merge`（`--merge` 露出無し）+ `kaji issue close`）
- [ ] **ラウンドトリップ確認**: `kaji issue view N --json body -q '.body'` の出力を `kaji issue edit N --body "$CURRENT_BODY"` で書き戻したときに本文が壊れない（Phase 2 段階では `gh` 透過のため bit-exact 維持）
- [ ] **既存 worktree 解決の維持**: `[issue-number]` → `[issue_id]` リネーム後も、`docs/247` / `fix/123` 等 prefix を伴う既存 Skill フローが既存挙動どおり worktree 解決できる（`[prefix]` を Skill 自前計算する経路が壊れていないこと）
...
- `make check` 緑、`make verify-docs` 緑、手動 smoke の 5 経路（受け入れ条件「手動 smoke（複数経路）」参照）完走確認
...
| 手動 smoke（複数経路: `issue-start` / `i-pr` / `pr-fix` or `pr-verify` / `issue-close`）| 2-B | 0.5 日 |
| **合計** | | **3.5 日** |
