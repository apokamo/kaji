...
- `kaji pr ...` は Phase 4 まで forge 専用の整理が未完了。現時点では `provider=local` でも legacy GitHub passthrough 側へ進み得るため、Phase 4 で明示エラー化する
...
`kaji issue edit ID --add-frontmatter KEY=VALUE` は frontmatter にユーザー拡張 key を追記する。ad-hoc な metadata（GitHub 復旧後の `migrated_to: <gh-number>` 等）を保持するための interface。**Phase 3-e 時点では未実装で、Phase 5 の `local-to-github-plan` / 復旧フローと一体で実装する。**
...
stateDiagram-v2
    ...
    Detected: 停止検知
    Detected --> LocalMode: kaji local init\nまたは config.local.toml で type=local
...
```
1. `.kaji/config.local.toml` の `[provider] type = "local"` override を削除する（または overlay を削除して tracked `.kaji/config.toml` の `type = "github"` に戻す）
...
既存 `.kaji/wf/feature-development.yaml` は `final-check -> i-pr -> end` で終端しており、`i-pr` は forge 上で PR を作成する Skill のため bare provider では動作しない。Phase 3-d で **local 用 workflow ファイルを追加済み**：
...
config の `provider.type` と workflow ファイルが整合しない呼び出し（例: `provider=github` で local 用 workflow を起動）を kaji 起動時に検証してエラー停止するかは Phase 4 の検討事項。Phase 3-e 時点では provider 解決と IssueContext 解決は fail-fast するが、workflow 名と provider type の対応検証はまだ独立した明示仕様になっていない。
...
| 4 | 次フェーズ | `kaji pr` / `pr-*` / `i-pr` を forge 専用として整理。`provider=local` では明示エラー化し、`pr_id` / `pr_ref` へ移行する。`build_prompt` の `IssueContext` required 化、workflow と provider type の整合検証も同 Phase 候補 |
| 5 | 後続 | `kaji sync from-github` / `kaji sync status` / `kaji sync local-to-github-plan`、`.kaji/cache/` 整備、`--add-frontmatter migrated_to=<gh-number>`、BCP runbook（コード同期戦略章を含む） |
| 別 Issue | 後続 | Windows native 対応。現時点は非対応で、Windows ユーザーは WSL を使う |

本設計は **GitHub 復旧を待たない buildout** を前提とする（§ 検証戦略の前提）。Phase 5（`kaji sync` 系）は GitHub アクセスが回復した時点で初めて実用価値が出るため後続に置くが、設計・実装自体は mock / fixture ベースで着手可能。
...
- Phase 4 で `kaji pr` / `pr-*` / `i-pr` をどの粒度で forge-only 化するか。CLI レベルの bare-provider エラー、Skill 冒頭の provider guard、workflow provider 整合検証の 3 層を分けて設計する必要がある
...
- `kaji config set` / `kaji config doctor` 等のセットアップ補助コマンドを追加するか。Phase 3 時点では `kaji local init` + fail-fast エラーメッセージで足りるが、BCP 復旧手順が増える Phase 5 では再評価する
