...
| フィールド | 現状の型ガード | 位置 |
|-----------|---------------|------|
| `exec` | あり（str / list[str] を argv へ正規化） | `workflow.py:78-112, 154` |
| `inject_verdict` | あり（bool） | `workflow.py:171-176` |
| `workdir` | あり（非空 str + 絶対パス） | `workflow.py:177-199` |
| `agent` | あり（str \| None） | `workflow.py:201-206` |
| `effort` | あり（str） | `workflow.py:208-223` |
| `timeout` | あり（正の int、bool 除外） | `workflow.py:225-236` |
| `id` | **なし**（`_STEP_REQUIRED_KEYS = ("id",)` は存在のみ検査） | `workflow.py:58, 132-138, 239` |
| `resume` | **なし** | `workflow.py:248` |
| `on` のキー | **なし**（`on` 自体が非空 dict であることのみ検査） | `workflow.py:158-170` |
| `cycle.entry` | **なし** | `workflow.py:292` |
| `cycle.loop` 要素 | **なし**（`loop` が list であることのみ検査） | `workflow.py:274-278, 294` |
| `cycle.on_exhaust` | **なし** | `workflow.py:295` |
...
本設計時に L1 の全フィールドを表（上記）で棚卸しし、型ガードの欠落が **`id` 系と verdict 系に限られる**ことを確認した。他フィールドは既に parse 時型検証を持つ。

`find_step()` を経由する参照解決系（遷移先 / resume / cycle entry / loop / tail）はすべて同じ str 前提を共有するが、これらは個別に壊れているのではなく型ガード欠落という単一原因の派生である。parse 境界で非 str を弾けば、下流は非文字列を考慮不要になる。
...
いずれも本設計時の実測・調査で確認済みであり、新規に発見した無関係な問題（`_shared/report-unrelated-issues.md` の報告対象）は無い。
...
- **既存 workflow を壊さない**: `.kaji/wf/*.yaml` の `id` / `resume` / `entry` / `loop` / `on_exhaust` はすべて非空文字列であることを本設計時に確認済み（`grep -rn "resume:\|entry:" .kaji/wf/*.yaml`）。既存 `tests/test_cli_validate.py::test_repository_workflows_all_validate` が回帰網となる。
