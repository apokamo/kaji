...
**スコープ**: 本 Issue のワークフロー YAML は **claude-only**（全ステップの agent が `claude`）で作成する。codex / gemini 用スキル（`.agents/skills/`）の整備は別 Issue とする。
...
| スキル | 常に注入 | 条件付き |
|--------|---------|---------|
| issue-create | `issue_number`, `step_id` | - |
| issue-start | `issue_number`, `step_id` | - |
| issue-design | `issue_number`, `step_id` | - |
| issue-review-design | `issue_number`, `step_id` | `cycle_count`, `max_iterations` |
| issue-fix-design | `issue_number`, `step_id` | `previous_verdict`, `cycle_count`, `max_iterations` |
| issue-verify-design | `issue_number`, `step_id` | `cycle_count`, `max_iterations` |
| issue-implement | `issue_number`, `step_id` | - |
| issue-review-code | `issue_number`, `step_id` | `cycle_count`, `max_iterations` |
| issue-fix-code | `issue_number`, `step_id` | `previous_verdict`, `cycle_count`, `max_iterations` |
| issue-verify-code | `issue_number`, `step_id` | `cycle_count`, `max_iterations` |
| issue-doc-check | `issue_number`, `step_id` | - |
| issue-pr | `issue_number`, `step_id` | - |
| issue-close | `issue_number`, `step_id` | - |

### 2. 全スキル共通：verdict ブロックの追加
