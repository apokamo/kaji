...
| issue-review-design | Approve | - | - | ABORT レベルの問題 |
...
| issue-review-code | Approve | - | BACK（設計に問題） | ABORT レベルの問題 |
...
### 3. review/verify スキル：verdict status と判定の対応

レビュー系スキルの既存判定（Approve / Changes Requested）を verdict status に対応付ける。

| 既存判定 | verdict status | 説明 |
|---------|---------------|------|
| Approve | PASS | 次ステップへ進む |
| Changes Requested | RETRY | fix ステップへ遷移（cycle の loop） |
...
- 全ステップの `skill` がファイルシステム上に存在するか（`dao_harness.skill.resolve_skill_path`）
