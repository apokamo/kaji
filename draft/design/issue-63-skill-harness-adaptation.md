...
| fix ステップ | resume 対象 | previous_verdict の内容 |
|-------------|------------|----------------------|
| fix-design | verify-design（2回目以降）/ review-design（初回） | レビュー/検証の指摘事項 |
| fix-code | verify-code（2回目以降）/ review-code（初回） | レビュー/検証の指摘事項 |
...
steps:
  ...
  - id: fix-design
    ...
    agent: claude
    resume: true   # 直前ステップのセッションを継続 → previous_verdict 注入
    on:
      PASS: verify-design
      ABORT: end
  - id: fix-code
...
`resume: true` により、ハーネスは `state.last_transition_verdict` を `previous_verdict` として注入する（`dao_harness/prompt.py` の実装に準拠）。
