...
一次情報（実世界障害ログ）: `.kaji-artifacts/184/runs/2605260007/run.log`
...
|----------|-------------------------------|---------------------------|
| `feature-development.yaml` / `-light` / `-local` / `full-cycle.yaml` | `BACK: design` | design 再起動 → ABORT（OB そのもの） |
| `implement-to-pr.yaml` | **BACK key 自体が無い**（`on:` は PASS/RETRY/ABORT のみ、L32-35） | `BACK` が injected valid_statuses に含まれず、skill が BACK を返すと `InvalidVerdictValue`（`verdict.py:208`）でクラッシュ |

両系統とも Step 1.4 に対して壊れているため、修正は全 5 workflow を対象とする。
...
| ファイル | 現状 | 追加 |
|----------|------|------|
| `feature-development.yaml` | `BACK: design` | `BACK_IMPLEMENT: implement` |
| `feature-development-light.yaml` | `BACK: design` | `BACK_IMPLEMENT: implement` |
| `feature-development-local.yaml` | `BACK: design` | `BACK_IMPLEMENT: implement` |
| `full-cycle.yaml` | `BACK: design` | `BACK_IMPLEMENT: implement` |
| `implement-to-pr.yaml` | （BACK 系なし） | `BACK_IMPLEMENT: implement` |
...
- skill は prompt 注入の valid_statuses（`step.on.keys()`）を権威とする。各 workflow が `BACK_IMPLEMENT` を `on:` に持つことで初めて skill が当該 status を発行できる。**全 review-code 採用 workflow への追加が漏れると、漏れた workflow で `InvalidVerdictValue` が発生する**ため、全 5 ファイルへの追加が必須制約
...
2. workflow YAML 5 ファイルの `review-code.on` に `BACK_IMPLEMENT: implement` を追加
3. `tests/workflows/test_builtin_workflows.py` に不変条件テストを追加（後述）
...
`tests/workflows/test_builtin_workflows.py` に以下を追加:

1. **review-code の BACK_IMPLEMENT routing 不変条件**:
   - 全 `.kaji/wf/*.yaml` を `load_workflow` でロード
   ...
   - これにより「Step 1.4 の差し戻し先が design ではなく implement である」「routing 先が実在する」を機械的に保証
...
Red→Green の確認: 修正前は review-code step に `BACK_IMPLEMENT` が無いためテスト 1 が FAIL（OB に対応）、5 workflow へ追加後 PASS（EB に対応）。OB を直接示す実ログ（run.log）が存在するため、bug.md escape clause により実装前合成 Red は実ログで代替し、恒久回帰テスト（修正後 Green）は本テストで担保する。

サイズ根拠: YAML パース + assertion のみ。外部プロセス・DB・ネットワーク無し。既存 `test_builtin_workflows.py` の `@pytest.mark.small` 慣例に整合。`docs/dev/testing-convention.md` のサイズ定義（純粋ロジック → Small）に従う。
...
- `tests/workflows/test_builtin_workflows.py`（全 workflow load）: BACK_IMPLEMENT 追加後も `validate_workflow` を通ること
...
| 実世界障害ログ | `.kaji-artifacts/184/runs/2605260007/run.log` | OB を直接示す。review-code が BACK → design 起動 → ABORT の event 列。bug.md escape clause の実ログ代替根拠 |
