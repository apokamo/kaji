...
`_resolve_run_issue_context()` 完了直後・`SessionState.load_or_create()` 完了直後に、以下の override / capture を追加:

1. **state → context override（load 直後）**:
   ```python
   ...
   ```
2. **context → state capture（main loop 内、各 step dispatch 直前に 1 度判定）**:
   ```python
...
- **Issue 本文 NOTE は parse しない**: 本修正案では Issue 本文 NOTE（`> [!NOTE]\n> **Worktree**: ...`）を override の source として使わない。理由は (a) 人間が手で改変可能で source of truth として脆い、(b) provider に文字列 parsing 責務を持ち込む、の 2 点。`git worktree list` の物理事実 + state 永続化で十分。
...
2. **`runner.run()` の 2 箇所修正**:
   - state load 直後の override（context の worktree/branch を state 値で置換、`dataclasses.replace` で immutable に再構築）。
...
- 偽 provider が `IssueContext(worktree_dir="/tmp/kaji-feat-218", branch_name="feat/218", ...)` を返す状態で、`SessionState` に `worktree_dir="/tmp/kaji-fix-218"` / `branch_name="fix/218"` が事前保存されていれば、`WorkflowRunner.run()` 起動後に `issue_context` が override されることを検証（runner 内部の override 経路を直接呼ぶ単体）。
...
- 修正範囲が runner + state の 2 module に閉じることを示す。
