...
EPIC `local-pc5090-4` 子 Issue #4。`local-pc5090-5`（GitLabProvider 実装）/
`local-pc5090-6`（`kaji issue/pr` GitLab passthrough + `gl:N` 規約）が完了済みで、
...
| `glab` CLI の `glab issue list --output-format details` を使う | `--output-format` は `details / ids / urls` のみで構造化 JSON を返さない（`local-pc5090-5` 設計書 § Primary Sources 参照）。安定 parse のため `glab api projects/<repo>/issues` REST を採用する |
...
| `httpx` で GitLab REST を直接叩く | `pyproject.toml:21` の `[project.dependencies]` に `httpx` 不在。CLI 戦略 (`local-pc5090-5` § 確定事項 #1) 通り `glab` subprocess 経由で統一する |
...
```text
local-pc5090-1  open    Title of local issue
local-pc5090-3  closed  Another local issue
...
# 統合表示
$ kaji issue list
local-pc5090-8  open    kaji sync from-gitlab + sync status + cache 自動 populate
...
| pagination 中で 1 ページ目以降の失敗 | 既書き込み分は残し（partial sync は cache に残る）、`.sync-meta.json` は **書かない**（最終 sync 時刻が嘘にならない） + exit 2 |
...
- `glab` CLI が PATH にある（`local-pc5090-5` で既に依存として確立）
...
- per_page=100 が GitLab REST API の上限（後述 § Primary Sources / `local-pc5090-5` 設計書
  の同節と同根拠）
...
- `kaji sync from-github` → 検証期間中は forge 採用先確定後の対応（`local-pc5090-12`）
- `kaji sync local-to-gitlab-plan` → bucket `local-pc5090-1` 残務
...
sync_from_gitlab(config, repo_override, quiet)
  ...
  │     └─> page=1, 2, ... until empty array
  ├─> _write_cache_files(issues, cache_dir) # atomic write per issue
  └─> _write_sync_meta(meta, sync_meta_path)# atomic write
```
...
`local-pc5090-5` の `_glab_api_get()` パターンを **再利用しない**。本 Issue の sync は
...
`--hostname gitlab.com` は `local-pc5090-5` の方針（self-hosted 非対応）と整合。
...
```python
def _write_cache_files(issues: list[dict], cache_dir: Path) -> None:
...
### closed 化された entry の保持

issue 本文「ローカル cache に存在するが GitLab 側で取得結果に含まれない Issue（=
既に closed 化された）は cache に残す」を満たすため、本実装は **「fetch 結果に
含まれない `gl-*.json` を delete しない」** という最も単純な戦略を採る:

- pagination で取得した全 open issues の `iid` set を作る
- cache dir の既存 `gl-*.json` を列挙し、set に **無い** entry は **そのまま残す**
- 残す entry の中身は **更新しない**（過去 sync 時点の snapshot を保持する）

実装上、これは「open 全件を上書き write して、それ以外は何もしない」だけで自然に
成立する。明示的な「closed 化検出 → 何かする」ロジックは不要。
...
def _list_cached_gitlab_issues(self, state: str, labels: list[str] | None) -> list[Issue]:
    ...
    for path in sorted(self._cache_dir_root.glob("gl-*.json")):
        ...
        # state 正規化: GitLab 'opened' → 'open'
        gl_state = issue_payload.get("state", "")
        normalized_state = "open" if gl_state == "opened" else gl_state
        if state != "all" and normalized_state != state:
            continue
...
`.sync-meta.json` は **全 page の fetch + cache write 完了後にのみ書く**（前述）。
途中失敗時は古い `.sync-meta.json` が残るが、そちらの方が「最終 sync 時刻が嘘に
ならない」という invariant を保つ。代替として「partial 失敗時は `.sync-meta.json`
を削除」も考えられるが、user が "elapsed = n/a" と "elapsed = 数日前" のどちらに
驚くかを天秤にかけて、**「数日前」の方が正しい情報量** と判断（最後に成功した
sync を保持）。
...
- `_write_cache_files()` が `gl-<iid>.json` を atomic write し、`schema_version=1`
  / `forge="gitlab"` / `fetched_at` を含む wrapper を生成する
...
- `list_issues()` が cache 由来の `gl:42` entry を末尾に append し、`Issue.id` が
  `"gl:42"` 形式である
- `list_issues(state="open")` が cache の `state="opened"` entry を採用、
  `state="closed"` entry を弾く
...
- `glab` を `subprocess.run` の monkeypatch で mock し、`sync_from_gitlab()` の
  ...
  - 既存 `gl-99.json`（前回 sync 時の closed 化された entry）が 100 件の fetch 結果に
    含まれない場合に **delete されない / 中身が変更されない**
  - `glab` が exit != 0 を返す → `SyncError` で停止し、`.sync-meta.json` は **書かれない**
  - 1 ページ目成功 + 2 ページ目で `glab` 失敗 → 1 ページ目分の cache は残る
    （partial sync）+ `.sync-meta.json` は **書かれない**（invariant 確認）
...
- `kaji issue list` の CLI 経由で:
  ...
  - cache に 5 件 GitLab issue 投入 → 末尾に `gl:1` 〜 `gl:5` 行が出る
  - `--state closed` で local closed + cache の closed のみ
...
- 本 Issue 範囲では **追加しない**。実 GitLab 通信 E2E（`make test-large-gitlab`）
  は子 Issue #6 (`local-pc5090-10`) の責務（EPIC OUT スコープ）。
...
| `docs/cli-guides/gitlab-mode.md`（子 Issue #5 で新設予定） | **なし（本 Issue では触らない）** | 子 Issue #5 (`local-pc5090-9`) の主成果物。本 Issue では先行追加しない |
| `docs/dev/workflow_guide.md` | **なし** | sync は workflow 概念に直接影響しない（CLI utility） |
| `docs/adr/` | **なし** | 既存 ADR の枠組み内（cache layout / glab CLI 戦略は `local-pc5090-5` で確立済） |
...
| **glab CLI: api command** | https://gitlab.com/gitlab-org/cli/-/blob/main/docs/source/api.md | `glab api <endpoint>` は GitLab REST v4 を起動し JSON を stdout に出す。`--hostname gitlab.com` で対象 GitLab を固定可能。**本設計の `_glab_api_get` の正本**（`local-pc5090-5` 設計書と同根拠） |
...
| **EPIC `local-pc5090-4` 確定事項** | `.kaji/issues/local-pc5090-4-epic-gitlab-validation/issue.md` § 確定事項 | 本設計の前提として参照。特に #1（CLI subprocess 必須化）/ #3（`gitlab.com` 固定 / `gl:N` 規約）/ #5（本格実装） |
| **`local-pc5090-5` 設計書** | `draft/design/issue-local-pc5090-5-gitlab-provider-impl.md` | GitLabProvider 既存実装の構造的整合点。本 Issue は `local-pc5090-5` が確立した glab CLI 戦略 / cache layout の方針 / config schema を再利用する |
