...
| ファイル | 現状 | 変更内容 |
|---------|------|---------|
| `pyproject.toml` | `license = {text = "MIT"}` + MIT classifier + `setuptools>=68.0` | SPDX 文字列 + classifier 削除 + `setuptools>=77.0.0` に引き上げ |
...
`python -m build` で sdist と wheel をビルドし、PEP 639 準拠のメタデータとライセンスファイル配置を検証する。

#### sdist 検証
- `kaji-*.tar.gz` 内に `LICENSE` ファイルが含まれること
- `PKG-INFO` に `License-Expression: Apache-2.0` が記載されていること
- `PKG-INFO` に `License-File: LICENSE` が記載されていること
- `PKG-INFO` の `Metadata-Version` が `2.4` 以上であること
