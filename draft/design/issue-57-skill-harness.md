...
- **スキル本体は各 PJ に配置**: `.claude/skills/` や `.agents/skills/` に Claude Code / Codex 形式で配置
...
- **セッション状態ファイル**: `test-artifacts/<issue>/<timestamp>/session-state.json` — 各ステップのセッション ID、verdict 履歴を保持（ステップ完了ごとに更新）
- **進捗ファイル**: `test-artifacts/<issue>/<timestamp>/progress.md` — 人間可読な進捗一覧（ステップ完了ごとに自動更新）
...
| CLI | 非インタラクティブ | ストリーミング出力 | resume 時 JSON | セッション ID 取得 |
|-----|-------------------|-------------------|---------------|-------------------|
| Claude Code v2.1+ | `-p` | `--output-format stream-json --verbose` | 可 | `session_id` フィールド |
| Codex v0.112+ | `exec` | `--json`（デフォルトで JSONL ストリーム） | **可**（v0.112 で解消） | `thread.started` → `thread_id` |
| Gemini v0.31+ | 位置引数 | `-o stream-json` | 可 | `init` → `session_id` |
...
ハーネスがプロンプト構築時にスキルファイルを読み込む際、`step.agent` に応じてパスを解決する:
...
def resolve_skill_path(skill_name: str, agent: str, workdir: Path) -> Path:
...
6. **タイムアウト・プロセス管理**: 各ステップにタイムアウト（デフォルト 1800s）を設定。タイムアウト時はプロセスを SIGTERM → 猶予後 SIGKILL で強制終了し、`StepTimeoutError` を raise する
...
| `previous_verdict` | str | No | 前回の verdict（reason + evidence）。fix/verify 系ステップで注入 |
...
スキルは実行完了時に以下の verdict ブロックを出力に含めること:

```
---VERDICT---
status: PASS | RETRY | BACK | ABORT
reason: "判定理由"
evidence: "具体的根拠（テスト結果、レビュー指摘、差分等）"
...
def run_workflow(workflow: Workflow, issue: int, workdir: Path, verbose: bool = True):
    ...
    artifacts_dir = Path(f"test-artifacts/{issue}/{datetime.now().strftime('%y%m%d%H%M')}")
    logger = RunLogger(artifacts_dir / "run.log")
    logger.log_workflow_start(issue, workflow.name, state.run_id)
    ...
    while current_step and current_step.id != "end":
        ...
        if cycle and state.cycle_iterations(cycle.name) >= cycle.max_iterations:
            ...
            prompt = build_prompt(current_step, issue, state, workflow, workdir)

            # 3. CLI を実行（ストリーミング + リアルタイムログ）
            session_id = state.get_session_id(current_step.resume) if current_step.resume else None
            step_log_dir = artifacts_dir / current_step.id
            ...
            result = execute_cli(
                ...
                session_id=session_id, log_dir=step_log_dir, verbose=verbose,
            )
...
def execute_cli(step: Step, prompt: str, workdir: Path,
    ...
    args = build_cli_args(step, prompt, workdir, session_id)
    ...
    # タイムアウト監視（別スレッドで SIGTERM → SIGKILL）
    timer = threading.Timer(timeout, _kill_process, args=[process, step.id, timeout])
    timer.start()
    try:
...
def _kill_process(process: subprocess.Popen, step_id: str, timeout: int):
...
def build_cli_args(step: Step, prompt: str, workdir: Path,
    ...
    match step.agent:
        case "claude":
            return _build_claude_args(step, prompt, workdir, session_id)
        case "codex":
            return _build_codex_args(step, prompt, workdir, session_id)
        case "gemini":
            return _build_gemini_args(step, prompt, workdir, session_id)


def _build_claude_args(step, prompt, workdir, session_id):
    ...
    args += ["--permission-mode", "bypassPermissions"]  # ハーネスはサンドボックス環境で実行前提
    args.append(prompt)
    return args


def _build_codex_args(step, prompt, workdir, session_id):
    if session_id:
...
def _build_gemini_args(step, prompt, workdir, session_id):
...
パース戦略（デリミタ形式 + CLI 出力フォーマット対応の2段）:
...
def _parse_fields(block: str) -> Verdict:
    """verdict ブロックから status / reason / evidence / suggestion を抽出。"""
    fields = {}
...
```
test-artifacts/<issue>/<YYMMDDhhmm>/
├── run.log                    # ワークフロー層ログ（JSONL）
├── session-state.json         # セッション状態（機械向け、ステップ完了ごとに更新）
...
class SessionState:
    ...
    def load_or_create(cls, issue: int, artifacts_dir: Path) -> "SessionState":
        ...
        if path.exists():
            ...
            return cls(**data, _artifacts_dir=artifacts_dir)
    ...
    def _persist(self):
        ...
        if self._artifacts_dir:
            ...
            path = self._artifacts_dir / STATE_FILE
            path.write_text(json.dumps(asdict(self), default=str, ensure_ascii=False))
    ...
    def _write_progress_md(self):
        ...
        for record in self.step_history:
            mark = "x" if record.verdict.status == "PASS" else " "
            lines.append(
                f"- [{mark}] {record.step_id}: {record.verdict.status}"
        ...
        path = self._artifacts_dir / "progress.md"
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
```
...
入力契約に基づき、ハーネスがスキル実行時にプロンプトへ注入する変数を構築:

```python
def build_prompt(step: Step, issue: int, state: SessionState,
                 workflow: Workflow, workdir: Path) -> str:
    skill_path = resolve_skill_path(step.skill, step.agent, workdir)
    ...
    # 前回の verdict（fix/verify 系ステップ用）
    last_record = state.last_record_for(step.id)
    if last_record:
        v = last_record.verdict
...
- **Verdict パーサ**: デリミタ抽出、4フィールド抽出（status/reason/evidence/suggestion）、verdict 未検出（VerdictNotFound）、status 値不正（InvalidVerdictValue — 回復不能）、必須フィールド欠損（VerdictParseError）、ABORT 時 suggestion 必須
- **CLI 別アダプタ**: 各アダプタ（Claude/Codex/Gemini）× extract_session_id / extract_text / extract_cost の組み合わせ。実際の CLI JSONL サンプルを入力としたデコード検証
- **CLI 引数ビルダー**: 各 agent（claude/codex/gemini）× 新規/resume × model/effort/max_budget_usd/max_turns/timeout の組み合わせ。agent 非対応フィールドが無視されることの検証を含む
- **スキルパス解決**: agent 別ディレクトリ（`.claude/skills/` vs `.agents/skills/`）の正しい解決、存在しないスキル（SkillNotFound）、パストラバーサル防御（SecurityError）
- **セッション状態管理**: save/load、サイクルイテレーションカウント、セッション ID 検索
- **サイクル上限判定**: `max_iterations` 到達時の `on_exhaust` verdict 発行
- **プロンプト構築**: 変数展開、スキル内容の埋め込み

### Medium テスト

- **CLI ストリーミング統合テスト**: モック CLI プロセスが JSONL を逐次出力する環境で、`stream_and_log()` の即時 flush・アダプタデコード・ファイル出力を検証。タイムアウト時の SIGTERM → SIGKILL 処理、CLI 未インストール時の CLINotFoundError も検証
...
| Codex CLI `exec --help` (v0.112.0) | ローカル実行結果 | resume 時に `--json`, `-m`, `--ephemeral`, `-o` が使用可能（v0.63.0 時点の制約が解消）。`-c key=value` で config.toml 値の CLI オーバーライドが可能（`-c 'model_reasoning_effort="high"'`）。`--help` に「Use a dotted path (foo.bar.baz) to override nested values. The value portion is parsed as TOML.」と記載 | `codex exec --help` 実行 | 2026-03-09 |
...
| Gemini CLI `--help` (v0.31.0) | ローカル実行結果 | `-o stream-json` で JSONL ストリーミング出力。`--allowed-tools` は非推奨で Policy Engine（TOML）への移行が推奨 | `gemini --help` 実行 | 2026-03-09 |
