...
- **失敗ケース**:
  | 条件 | 振る舞い |
  |------|---------|
  | `git worktree list` が exit != 0（非 git repo / git CLI 不在） | `LocalProviderError` を raise。原因と「`provider.local` 設定下では git repo が必須」である旨を message に含める |
  ...
  | porcelain 出力が parse 不能 | `LocalProviderError` を raise |
...
| `docs/ARCHITECTURE.md` | あり | LocalProvider の `repo_root` セマンティクスが「cwd 起点 worktree」→「default_branch worktree 固定」に変わる旨を 1 段落追記 |
| `docs/dev/` | あり | `development_workflow.md` / `workflow_guide.md` で `kaji issue comment local-*` を呼ぶ手順に「cwd 不問」を明記 |
...
| `.claude/skills/*/SKILL.md` | あり | `/issue-close` / `/issue-design` / `/issue-fix-code` 等で `kaji issue comment local-*` を呼ぶ箇所に「main worktree への cd は不要」を反映。具体対象は実装時に grep で網羅 |
