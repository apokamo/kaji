...
5. **section/key 仕様表**: 各 key について `必須/任意`・`型`・`既定値`・`検証規則`・`挙動`
   ...
   | `[paths]` | `worktree_prefix` | 任意 | str | `""`→`"kaji"` | `config.py:134-140`, `436-452` |
...
- `[paths] worktree_prefix = "kaji"` を明示追加（現行の fallback 値と一致）。
...
> **不変条件**: 追加する明示値はすべて現行の暗黙 default と同値であり、`KajiConfig`
> の load 結果（parse 後の dataclass）は変更前後で不変。すなわち実行時挙動は変わらない。
...
- **挙動非変更**: `.kaji/config.toml` に追加する明示値は現行の暗黙 default と同値
  （§ B 不変条件）。config loader / provider の挙動は変えない（スコープ外）。
...
3. `.kaji/config.toml` を編集（§ B）。追加値が default と同値であることを `config.py`
   の default 定義と突合して確認。
...
docs-only / metadata-only（`.kaji/config.toml` の内容変更を含むが、追加値は既存の
暗黙 default と同値であり実行時挙動は不変）。
...
- **挙動非変更の根拠確認**: 追加する明示値（`worktree_prefix="kaji"` /
  `agent_runner="headless"` / `git_remote="origin"`）が `config.py` の各 default
  （`24`/`32`/`51`, `447`/`233`/`340`）と一致することを突合し、parse 後の dataclass が
  不変であることを示す。

### 恒久テストを追加しない理由（`docs/dev/testing-convention.md` の 4 条件）

1. 独自ロジックの追加・変更を含まない（docs 整理 ＋ default と同値の明示化のみ）。
2. 想定不具合（リンク切れ）は `make verify-docs`、config 構文/key 不正は既存 config
   loader テスト群＋ load 時 `ConfigLoadError` で捕捉済み。
3. 明示値＝既存 default のため、新規テストを足しても回帰検出情報が増えない。
