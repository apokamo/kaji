...
- **ADR 008（後方互換レイヤ非提供）と整合させる**: `base` キーを持たない既存 YAML が従来経路のまま
  ...
  ではない。O1 ではこの区別を明示する。
- **ADR 010（Pydantic による外部入力検証）との関係を明記する**: overlay YAML は新規の外部入力契約に
  当たるため、schema 検証を Pydantic で書くか既存 dataclass validator に合わせるかは後続 Issue の
  論点として O1 に残す（本 Issue では決めない）。
...
| Q3 | resolved workflow の事前可視化に必要な要件（何を出せば「実効値の確認」が成立するか） | `commands/validate.py:51-96`（現行 `kaji validate` は L1/L2/L3 の可否のみ出力）、`docs/dev/workflow-authoring.md:540-560` | 「差分 YAML の暗算をやめられる」（Issue ユースケース 4）を満たす最小出力＝全 step の解決後 `agent`/`model`/`effort`/`timeout`/`workdir` と、値の由来（base / defaults / steps）を要件として抽出する。CLI 名・出力形式は後続 Issue の設計対象として明示的に未確定のまま残す | O1 §「決定」resolved 可視化の要件 |
...
3. **決定**: GO / NO-GO / 保留。GO の場合は schema・merge 規則（3 層）・制約（単層・scalar のみ・
   `base` の信頼境界）・validate 診断・resolved 可視化の要件・artifact 要件を確定形で記す
...
6. **判断を変える条件**: この決定を再検討すべき観測（例: topology 差分を要する variant の出現、
   overlay の解決コスト起因の障害）
7. **未確定事項**: 後続 Issue で決める論点（CLI 表面、artifact schema、Pydantic 採否）
...
| 4 | resolved 可視化と artifact 保存の要件 | O1 §「決定」 | Q3 / Q4 の要件が、CLI 名・schema を確定しない粒度で書かれている |
...
| GO / NO-GO の決定主体 | research の成果として判断し、事前に固定しない | owner `grill-me provenance` コメント §「残した AI 仮定」（人間決定） | Q7 の 4 軸比較として判定手順を具体化。保留も許容し、保留を解く条件の記載を義務化 |
...
| resolved の事前可視化 | run 前に表示する公開 CLI 機能を追加する | Issue 本文 `## 決定事項`（人間決定） | Q3 で「要件のみ確定、CLI 名・出力形式は後続 Issue」と境界を設定。owner コメントが `kaji validate --resolved` を AI 仮定として残した扱いを踏襲 |
...
| Workflow / Step モデル | `kaji_harness/models.py:46-112` | `Step` / `Workflow` は dataclass。overlay を load 時解決にすればこのモデルは無変更で済む（Issue 本文の制約と一致） |
...
| 外部入力の Pydantic 検証 | `docs/adr/010-pydantic-series-input-validation.md` | 「Existing workflow parsing is unchanged; migrating that established contract is outside this decision」。overlay schema を Pydantic で書くかは未決論点として O1 に残す |
