...
class LocalProvider:
    ...
    def _cache_dir_root(self) -> Path:
        """``.kaji/cache/`` 直下。``gh-*.json`` / ``.sync-meta.json``。

        旧 ``.kaji/cache/issues/<n>.json`` layout を廃止し、
        GitHub cache を ``gh-<n>.json`` に正規化した（issue #34）。
        """
    ...
    def resolve_issue_context(self, issue_id: str) -> IssueContext:
        ...
        return IssueContext(
            ...
            worktree_dir=build_worktree_dir(prefix, issue_id, self.repo_root, self.worktree_prefix),
    ...
    def view_cached_issue(self, number: str) -> Issue:
        ...
        を案内）。旧 ``.kaji/cache/issues/<n>.json`` layout から
