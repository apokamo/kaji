...
# Record a transcript only when a util-linux compatible script(1) is available.
# util-linux's script understands the GNU long options below; the BSD / macOS
# native script does not, so we fall back to running the agent directly rather
# than failing before launch (fail-soft).
run_with_transcript() {
  ...
  echo "WARNING: util-linux script(1) not found; terminal transcript will not be recorded" >&2
  # Non-login shell: keep PATH resolution consistent with the transcript branch
  # (util-linux script runs the command via a non-login `sh -c`), and rely on the
  # PATH that the terminal inherited rather than re-sourcing login profiles.
  exec bash -c "$shell_command"
}
