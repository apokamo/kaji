...
- **移行対象**: `local-pc5090-1` 〜 `local-pc5090-20`（20 件）
...
- [ ] 1〜20 の issue ディレクトリ (20 件) が `local-p1-N-*` に rename されている
- [ ] **21 (本 issue) は `local-pc5090-21-*` のまま保持されている**（移行境界の歴史的記録）
- [ ] 142 コメントファイル (1〜20 配下) が新形式 + `-p1.md` に rename されている
- [ ] 21 配下のコメント（本 issue 自身のもの）は新形式 + `-pc5090.md` のまま保持されている
- [ ] `.kaji/counters/p1.txt` が存在し、中身は `21`（次の新規 issue は local-p1-22）
- [ ] `.kaji/counters/pc5090.txt` が **削除** されている
- [ ] `.kaji/config.local.toml` の machine_id が `p1`（**migration commit 群の最後**で変更）
...
- [ ] 移行後に `kaji issue create` で `local-p1-22` が採番されることを smoke test で確認
