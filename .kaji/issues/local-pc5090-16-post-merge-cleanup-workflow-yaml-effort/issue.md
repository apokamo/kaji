...
> **Branch**: `feat/local-pc5090-16`

## 設計書

<details>
<summary>設計書全文（draft/design/issue-local-pc5090-16-post-merge-cleanup-effort-and-commit.md）</summary>
...
| テスト名 | サイズ | 検証内容 |
|----------|--------|---------|
| `test_commit_local_issue_change_message_format` | Small | helper が生成する commit message が `chore(local): comment for <issue_ref>` 形式（local provider なら bare ID `local-pc5090-16`、github provider なら `#<n>`。`_format_issue_ref` 経由） |
...
| local issue_ref 整形 | `kaji_harness/state.py::_format_issue_ref` | local provider の issue_ref が bare ID（`#` 無し）で整形される出典（commit message が `chore(local): comment for local-pc5090-16` 形式になる根拠） |
...
local-pc5090-5/14 完了後の workflow ログ調査で発見した 2 つの独立バグを bundle として扱う:
...
`.kaji-artifacts/local-pc5090-5/runs/2605091602/run.log:5` の実害:
...
- レビューコメント: `.kaji/issues/local-pc5090-16-post-merge-cleanup-workflow-yaml-effort/comments/0001-pc5090.md` の 3 件 Findings
- 関連 commit:
  ...
  - `3501fc2` chore(local): add design doc and review comments for local-pc5090-14 — Issue 14 close 救済 commit (2)
- 関連 Issue:
  - `local-pc5090-15` make check が main で 10 件 fail（別系統、Issue 5 fixture 漏れ + テスト config 隔離欠落）
  - `local-pc5090-17` issue-start skill のデフォルト prefix が type:* label 由来の branch_prefix と不整合（同調査由来）
- 1 次情報（実行ログ）:
  - `.kaji-artifacts/local-pc5090-5/runs/2605091602/run.log:5` (effort `High` reject の error)
  - `.kaji-artifacts/local-pc5090-14/runs/2605091631/close/console.log:5-12` (close 時の未コミット蓄積)
...
- 関連ドキュメント:
  ...
  - 設計書配置予定: `draft/design/issue-local-pc5090-16-post-merge-cleanup-effort-and-commit.md`
